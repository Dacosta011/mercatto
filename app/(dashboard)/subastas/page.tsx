...
function VotingCard({
  ...
  const handleAdminEndVoting = async () => {
    if (!adminToken) return;
    setEndingVote(true);
    const res = await fetch(`/api/tournaments/${code}/auctions/${a.id}`, {
      method: "PATCH",
    ...
    <section className="mb-10">
      ...
      {adminToken && (
        <div className="mt-4 flex justify-end">
          <button
            onClick={handleAdminEndVoting}
            disabled={endingVote}
            className="px-4 py-2 rounded-xl border border-[#F59E0B]/30 text-[#F59E0B] text-sm font-medium hover:bg-[#F59E0B]/10 transition-colors cursor-pointer disabled:opacity-50 flex items-center gap-2"
          >
            {endingVote ? (
              <>
                <span className="w-3.5 h-3.5 border-2 border-[#F59E0B]/30 border-t-[#F59E0B] rounded-full animate-spin" />
                Finalizando…
              </>
            ) : (
              <>
